...
public class FeedAnimalDispenserBehavior extends OptionalDispenseItemBehavior {
    ...
    private static ItemStack eatItem(Animal animal, ItemStack foodStack) {
        ...
        if(type == EntityTypes.CAT) {
            Holder<CatSoundVariant> csv = animal.get(DataComponents.CAT_SOUND_VARIANT);
            if (csv != null) {
                if (animal.isBaby()) {
                    animal.playSound(csv.value().babySounds().eatSound().value(), 1.0F, 1.0F);
                } else {
                    animal.playSound(csv.value().adultSounds().eatSound().value(), 1.0F, 1.0F);
                }
            }
        }
